...
static struct option longopts[] = {
  {"bind-config-directory", required_argument, 0, 'd'},
...
int main(int argc, char *argv[]) {
  ...
  bool verbose=false;  
  int long_index = 0;
  while ((opt = getopt_long(argc, argv, "d:n:w:c:hVvt:", longopts, &long_index)) != -1) {
    switch (opt) {
      case 'd' : strcpy(zonedirectory, optarg);
        break;
  ...
  DIR *zonedir;
  zonedir = opendir(zonedirectory);
  if (NULL == zonedir) {printf("CRITICAL: Couldn't open directory!"); exit(CRITICAL);}
  char *files = get_zones(zonedir);
...
char *get_zones(DIR *zonedir) {
  ...
  while ((dir = readdir(zonedir)) != NULL) {
